-- Transformice #anvilwar module loader - Version 2.31
...
lang.br = {
	...
	bar = "<b>#anvilwar</b> RTM 21340.151",
...
function removeImages()
	for _,f in next,{"winBlueImg","winRedImg","doubleKillImg","multiKillImg","startingImg"} do
		tfm.exec.removeImage(f)
	end
end
function changeMap()
...
function eventLoop(passado,faltando)
	...
	if reset == true then
		if faltando <= 10000 and faltando >= 9490 then
			tfm.exec.removeImage(winBlueImg)
			tfm.exec.removeImage(winRedImg)
		end
		if faltando < 1 then
			changeMap()
		end
		...
		if count_azul > 0 and count_vermelho > 0 then
			if count == 0 then
				for name,player in pairs(tfm.get.room.playerList) do
					...
					startingImg = tfm.exec.addImage("172b36628e4.png","&1",125,100,nil)
					if data[name] then
						if data[name].team == "azul" then
			...
			count=count+0.5
			if count == 4 then
				tfm.exec.removeImage(startingImg)
			end
			if count >= 10 then
		...
		if loop == 0 then
			...
			if tfm.get.room.community == "br" then
				if kills == 2 then
					...
					doubleKillImg = tfm.exec.addImage("172b3661172.png","&1",125,100,nil)
				elseif kills >= 3 then
					tfm.exec.chatMessage("<ROSE>Multi Kill: "..kills.." kills de "..actual_player.."!")
					multiKillImg = tfm.exec.addImage("172b3664054.png","&1",125,100,nil)
				end
			else
				if kills == 2 then
	...
	if valendo == true and reset == false then
		...
		ui.addTextArea(2228,"",nil,260,14,340,85,0x010101,0x010101,0.5,true)
...
function eventMouse(name,x,y)
	...
	if data[name].clicked == false then
		...
		tfm.exec.removeImage(splashDisplay)
	end
end
...
function eventNewPlayer(name)
	...
	splashDisplay = tfm.exec.addImage("172b365e290.png","&1",125,100,name)
end
for name,player in pairs(tfm.get.room.playerList) do
	eventNewPlayer(name)
...
function winRed()
	...
	advanceLevel()
	winRedImg = tfm.exec.addImage("172b36657c5.png","&1",125,100,nil)
end
function winAzul()
	for name,player in pairs(tfm.get.room.playerList) do
		if data[name].team == "azul" then
	...
	advanceLevel()
	winBlueImg = tfm.exec.addImage("172b365fa02.png","&1",125,100,nil)
end
...
function eventLoop(passado,faltando)
	...
	if active > 0 and faltando < 1 and rato >= 2 then
		...
		if active == 61 then
			for name,player in pairs(tfm.get.room.playerList) do
				if player.x > xpos-20 and player.x < xpos+100 then
					tfm.exec.killPlayer(name)
				end
			end
		end
...
tfm.exec.chatMessage("<VP>#anvilwar Multiple Module Loader revision 2<br>Version 2.31<br>By Spectra Advanced Module Group")
