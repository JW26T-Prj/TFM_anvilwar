-- Transformice #anvilwar module loader - Version 2.232
...
Version : RTM 50216.199
Compilation date : 10/09/2022 18:13 UTC
...
lang.br = {
	...
	legacy = "<J><b>Fuja do tubarão (shaman) e sobreviva dentro do oceano!</b><br><VP>/sala #anvilwar00watercatch<br><br><J><b>Um mapa-script tipo village, porém de praia!</b><br><VP>/sala #anvilwar00beach<br><br><J><b>Um mapa-script para quem gosta de aventuras radicais e natureza!</b><br><VP>/sala #anvilwar00naturalpark<br><br><J><b>Teste suas habilidades de WJ escalando a montanha!</b><br><VP>/sala #anvilwar00mountain<br><br>",
...
function showLobbyText(name)
	ui.addTextArea(402,"<p align='center'><font size='13'><b><font face='Courier New'><i>"..text.version.." RTM 50216.199 - "..text.comp_date.."10/09/2022 18:13 UTC - "..text.uploaded.."Morganadxana#0000</i>",name,-10,380,820,36,0,0,1.0,true)
end
...
function updateTextBar()
	if mode == "end" then
		...
		ui.setMapName("<N><b>#anvilwar</b>   <G>|   <VP>"..text.version.." <b>RTM 50216.199</b> <R>   <G>|   <N>"..text.mices_room.."<V><b>"..mices.."</b><")
	end
end
	...
	if mode == "lobby" then
		if data[name] then
			if data[name].ranking >= 0 then
				showImage(name,"1835da9d15e.png",0,16,1.0,0.96)
				showTeams(name)
				showLobbyText(name)
...
function eventNewGame()
	...
	if mode == "wait1" then
		...
		moveTeams()
		for name,player in next,tfm.get.room.playerList do
			data[name].scoreboard_id = tfm.exec.addImage("1835da984ec.png", ":1", 250, 15, name, 1.0, 1.0, 0, 1)
...
function eventChatCommand(name,command)
	...
	if command == "changelog" then
		showMenu(name,0xa8f233,140,110,520,100,"#anvilwar Changelog - RTM 50216.199","• Some minor changes")
	end
...
function eventLoop(passed,remain)
	...
	if mode == "wait2" or mode == "wait3" or mode == "shoot" then
		...
		if s >= 10 then
			ui.addTextArea(5652,"<font size='24'><p align='center'><font color='#050505'><b>"..m..":"..s.."",nil,337,24,100,40,0,0,1.0,true)
			ui.addTextArea(5651,"<font size='24'><p align='center'><b>"..m..":"..s.."",nil,335,22,100,40,0,0,1.0,true)
		else
...
function showWater(name)
	...
	tfm.exec.addImage("183b4bf815b.png","?1",4908,785,name)
	tfm.exec.addImage("183b4bf815b.png","!1",4908,785,name,1,1,0,0.65)
	for o=1,3 do
...
function checkAlivePlayers()
	if alives <= 0 then
		...
		if data[shaman] then
			...
			if data[shaman].dead == false then
				shaman=""
				tfm.exec.setGameTime(15)
...
function defineShaman()
	...
	for name,player in next,tfm.get.room.playerList do
		if tfm.get.room.playerList[name].score >= max_score and not tfm.get.room.playerList[name].isShaman then
			...
			showMessage("<ROSE>Não esqueça de se mover, ou você perderá sua vez de shaman!",name)
			showMessage("<ROSE><b>"..shaman.."</b> é o shaman! Saia de perto dele do jeito que for possível!")
...
function eventNewPlayer(name)
	if changed == true then
		...
		newData={
	["o"]=99; ["i"]=0; ["t"]=0; ["c"]=0; ["opened"]=false; ["moved"]=0; ["imaget"]=5; ["intoWater"]=false; ["shark_id"]=0; ["shark"]=0; ["dead"]=true; ["active_imgs"]={};
	};
		data[name] = newData;
		ratos=ratos+1;
...
function eventSummoningStart(name,id,x,y)
	...
	if not id == 17 and cannons >= 1 then
		showMessage("<R>Apenas é permitido o uso de canhões! Invocar outros objetos fará com que você morra e a partida termine!",name)
	end
end
...
function reset()
	if ratos >= 2 then
		...
		for n,p in next,tfm.get.room.playerList do
			if n:sub(1,1) == "*" then
				...
				data[n].o=99; data[n].moved=0; data[n].i=0; data[n].t=0; data[n].c=0; data[n].opened=false; data[n].dead=false;
	...
	if rawlen(tfm.get.room.objectList) > 0 then
		for id in next,tfm.get.room.objectList do
...
function eventLoop(p,r)
	...
	if mode == "game" or mode == "hide" then
		...
		if time_passed % 2 == 0 then
			for id in next,tfm.get.room.objectList do
				if tfm.get.room.objectList[id].type == 54 then
					tfm.exec.killPlayer(shaman)
					showMessage("<ROSE>O uso do gelo não é permitido neste jogo.")
					tfm.exec.removeObject(id)
					break
				end
			end
					...
					if tfm.get.room.playerList[n].y < 500 then
						...
						if shaman == n then
							...
							elseif tfm.get.room.playerList[n].y > 2500 and tfm.get.room.playerList[n].y <= 3660 then
								data[n].o=data[n].o-0.5; data[n].c=0;
							elseif tfm.get.room.playerList[n].y > 3700 then
								data[n].o=data[n].o-0.55; data[n].c=0;
							end
						else
							if tfm.get.room.playerList[n].y <= 1700 then
...
tfm.exec.chatMessage("<ROSE><b>#anvilwar</b> Multiple Module Loader revision 2<br>Version 2.232<br>By Morganadxana#0000")
