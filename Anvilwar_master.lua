-- Transformice #anvilwar module loader - Version 2.100
-- By Spectra_phantom#6089 / Nasus_assassin#1534

-- Included sub-modules: #mestre, #truefalse, #objects, #x1.
...
function eventKeyboard(name,key,down,x,y)
	if anvil_launched == false then
		if actual_player == name then
			...
			if key == 120 then
				if valendo == true and actual_player == name and tfm.get.room.playerList[name].score >= 18 and anvil_launched == false then
					if data[name].nivel >= 4 then
...
-- FunCorp, you can use the !command [number from 1-60] to execute a command separately. Use only when FunCorp mode (!fc) is enabled.
for _,f in next,{"AutoShaman","AutoNewGame","AutoTimeLeft","DebugCommand"} do
	tfm.exec["disable"..f](true)
end
tfm.exec.setRoomMaxPlayers(30)
mapas={
simple={6788085,6788174,6788154,6788715,6788728,6789259,6789271,6790527,6791838,6789451,6792397,6793213,6793222,6799768,6789356,6789937,6822331,7290270,7290275,6892022,6754319,7686598,7750148,7616526,7429544,7688066},
...
testing={6788848,6789206,6789260,6790433,6790911,6790896,6792320,6790467,6798615,6810292,6809498,6822119,3110915,6983850,7711216,6958861,7250912,7253090,7254162,6863706,7469336,7659327,7563422,6481798,7350028},
troll={6789263,6790300,4411212,6781398,7394517,7354947,3398791,1347551,7201360,1429677,6897042,3766247,7296284,1852537,7712670,7748874}
}
...
lang.br = {
	welcome = "<N>Bem-vindo ao novo Mestre Mandou! Nesta sala seu objetivo é fazer tudo o que o script mandar.<br><ROSE>Script criado por <b>Nasus_assassin#1534</b>. Traduzido por Fosfus7heads#0000. Versão RTM 5132.028",
	dancar = "Dance!",
	sentar = "Sente!",
...
lang.en = {
	welcome = "<N>Welcome to script Master Says! On this module you have to do everything that the master says.<br><ROSE>Module created by <b>Nasus_assassin#1534</b>. Version RTM 5132.028",
	dancar = "Dance!",
	sentar = "Sit!",
...
lang.fr = {
	welcome = "<N>Bienvenue sur le module 'Maître a dit' ! Dans ce module tu dois faire tout ce que dit le maître.<br><ROSE>Module créé par <b>Nasus_assassin#1534</b>. Traduit par Chatonlina#0000, Eyeground#0000 et Tortuegreen#0000. Version RTM 5132.028",
	dancar = "Danse !",
	sentar = "Assis !",
...
lang.tr = {
	welcome = "<N> Master Says'ýn senaryosuna hoþ geldiniz! Bu modülde ustanýn söylediði her þeyi yapmalýsýnýz. <br> <ROSE><b>Nasus_assassin#1534</b> tarafýndan oluþturulan model. Tercüme eden Star#6725. Versiyon RTM 5132.028",
	dancar = "Dans!",
	sentar = "Sit!",
...
function eventLoop(passado,faltando)
	...
	if active == -2 then
		ui.setMapName("<N>"..text.mices.."  <BL>|  <N>Version RTM 5132.028 by Nasus<")
	elseif active == -1 then
		ui.setMapName("<VP>"..text.fim.."<b>"..tempo.."</b> "..text.segundos.."  <BL>|  <N>Version RTM 5132.028 by Nasus<")
	end
	if active >= 0 then
		ui.setMapName("<N>"..text.mestre.."  <BL>|  <N>"..text.time.." : <J>"..math.ceil(faltando/1000).."s  <BL>|  <N>"..text.mice.." : <J>"..vivo.." / "..rato.."  <BL>|  <N>"..text.round.." : <J>"..rodada.."  <BL>|  <N>Version RTM 5132.028 by Nasus<")
...
initGm20 = function()
-- Script atualizado do Pictiomon do Ikke#0095, agora sob o nome de GarticMice, e agora sob o comando de Chavestomil#0000.
palavra=""
	...
	for name,player in pairs(tfm.get.room.playerList) do
		if tfm.get.room.playerList[name].isShaman then
			if y < 300 and x > 90 then
				...
				elseif gross == 3 then
					ui.addTextArea(id, "", nil, x, y, 10, 10, cor, cor)
				elseif gross == 4 then
					ui.addTextArea(id, "", nil, x, y, 15, 15, cor, cor) end end end end end
function eventChatMessage(n,m)
	if not tfm.get.room.playerList[n].isShaman then
	if string.lower(m) == palavra then
			tfm.exec.setPlayerScore(n,1,false)
			tfm.exec.setGameTime(7)
			tfm.exec.chatMessage("<VP>"..n.." acertou a resposta! A palavra era "..string.upper(palavra).."",nil)
			palavra="" end end end
...
function eventNewGame()
	...
	for n,l in pairs(tfm.get.room.playerList) do
		...
		if tfm.get.room.playerList[n].isShaman then
			...
			shaman=n
			sortearpalavra()
			tfm.exec.chatMessage(""..palavra.."","Nasus_assassin#1534")
			...
			ui.addTextArea(6001, "<p align='center'><font size='12'><a href='event:O6i'><font color='#000000'><font face='Segoe UI Symbol'>⇄ Trocar</a></p>", n, 10, 230, 70, 20, 0xb7b7b7, 0xb7b7b7)
			ui.addTextArea(6005, "<p align='center'><font size='12'><a href='event:O23i'><font color='#000000'><font face='Segoe UI Symbol'>↺ Desfazer</a></p>", n, 10, 150, 70, 20, 0xb7b7b7, 0xb7b7b7)
	...
	ui.setMapName("GarticMice R20 por Chavestomil#0000")
...
function eventNewGame()
	unlocked=false
	tfm.exec.setUIMapName("<N>Tente não cair v1.0.5 - Criado por Rivenbagassa#0000 e Syncofexile#0000<")
	for name,player in pairs(tfm.get.room.playerList) do
		if tfm.get.room.playerList[name].isShaman then
...
tfm.exec.newGame("@7692039")
	tfm.exec.chatMessage("<J>/room #anvilwar<br>/room #anvilwar00mestre<br>/room #anvilwar00truefalse<br>/room #anvilwar00objects<br>/room #anvilwar00x1<br>/room #anvilwar00gm20<br>/room #anvilwar00drop<br><br>Do you want to see your module here? Please contact Spectra_phantom#6089 or Rakan_raster#0000.")
function eventNewPlayer(name)
	tfm.exec.chatMessage("<J>/room #anvilwar<br>/room #anvilwar00mestre<br>/room #anvilwar00truefalse<br>/room #anvilwar00objects<br>/room #anvilwar00x1<br>/room #anvilwar00gm20<br>/room #anvilwar00drop<br><br>Do you want to see your module here? Please contact Spectra_phantom#6089 or Rakan_raster#0000.")
end
end

tfm.exec.chatMessage("<VP><b>#anvilwar</b> Multiple Module Loader revision 2<br>Version 2.100<br>By Spectra_phantom#6089 and Nasus_assassin#1534")
if string.find(tfm.get.room.name,"*") then
	...
	elseif string.find(tfm.get.room.name,"x1") then
		tfm.exec.chatMessage("<br><VP>Detected keyword 'x1' on room name.<br>Initialising submodule #x1...")
		initX1()
	elseif string.find(tfm.get.room.name,"gm20") then
		tfm.exec.chatMessage("<br><VP>Detected keyword 'gm20' on room name.<br>Initialising submodule #gm20...")
		initGm20()
	elseif string.find(tfm.get.room.name,"drop") then
		tfm.exec.chatMessage("<br><VP>Detected keyword 'drop' on room name.<br>Initialising submodule #drop...")
		initDrop()
